...
-- Functions to manage users, roles, and system administration
-- =====================================================
    ...
    IF user_info.role = 'instructor' THEN
        SELECT json_agg(
            json_build_object(
                'id', c.id,
                'title', c.title,
                'status', CASE WHEN c.created_at IS NOT NULL THEN 'published' ELSE 'draft' END,
...
GRANT EXECUTE ON FUNCTION admin_get_user_details(UUID, UUID) TO authenticated;
...
-- Success message
SELECT 'Admin Functions Created Successfully! Users will now appear in the admin panel.' as message;